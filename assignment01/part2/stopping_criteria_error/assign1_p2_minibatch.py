...
NUMITR = 350        #number of iterations
B_SIZE = 5          #batch size
...
def run():
    ...
    print "Running..."
    [w1, w2, b, l] = mini_batch_gradient_descent_runner(points, initial_w1, initial_w2, initial_b, learning_rate, num_iterations)
    ...
    anywhere = l * NUMEXAMPLES / B_SIZE
    plt.xlabel('(Total number of epochs) * (Number of rows in dataset) / (Batch size) (specifically it is: ' + str(anywhere) + ')')
    plt.ylabel('Error')
    plt.title('mini-batch gradient descent (learning rate:' + str(learning_rate) + ', batch size:' + str(B_SIZE) + ', Epoch Recorded:' + str(l) + ')')
