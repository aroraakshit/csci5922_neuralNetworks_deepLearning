...
#Implementing Stochastic gradient descent for linear regression using numpy.
...
def sgd_runner(points, starting_w1, starting_w2, starting_b, learning_rate, num_iterations):
    ...
    for l in range(num_iterations):
        ...
        for i in range(len(points)):
            ...
            W2[l,i] = w2
            B[l,i] = b
            if (error2(w1,w2,b,points) <= ERR_ALLOWED):
                print "number of epochs recorded!: " + str(l)
                return [w1, w2, b, l]
...
def run():
    ...
    print "Running..."
    [w1, w2, b, l] = sgd_runner(points, initial_w1, initial_w2, initial_b, learning_rate, num_iterations)
    print "After {0} iterations w1 = {1}, w2 = {2}, b = {3}, error w.r.t the whole dataset (for comparisons) = {4}".format(num_iterations, w1, w2, b, error2(w1,w2,b,points))
    plt.plot(ERROR.flatten()) #working!
    anywhere = l * NUMEXAMPLES
    plt.xlabel('(Total number of epochs) * (Number of rows in dataset) (specifically it is: ' + str(anywhere) + ')')
    plt.ylabel('Error')
    plt.title('online gradient descent (learning rate:' + str(learning_rate) + ', Epoch Recorded:' + str(l) + ')')
    print "Total iterations (number of times gradient_step was executed):" + str(len(ERROR.flatten()))
