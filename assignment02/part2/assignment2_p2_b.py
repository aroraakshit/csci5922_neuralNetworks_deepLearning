#Assignment 2 - Question 2 (B)     Akshit Arora
...
def run():
    ...
    counts = np.bincount(points_training[:,7].astype(dtype=int))
    print "Baseline: " + str(np.argmax(counts)) + ". Since its frequency is: " + str(counts[0]) + " in a training set with: " + str(len(points_training)) + " examples"
    ...
    baseline_target = np.zeros(len(points_training))
    baseline_current = current_value(points_training[:,2:7],W_1,b_1,W_2,b_2)
    baseline_error = np.sum(abs(baseline_current - baseline_target))/len(points_training) * 100
    ...
    #visualizing results
    plt.title("Error on training set (green) and baseline (red).")
    plt.xlabel('Number of Epochs')
    plt.ylabel('Error (% examples misclassified)')
    plt.plot(ERROR_training,'g')
    plt.plot([0,20],[baseline_error,baseline_error], color='r')
